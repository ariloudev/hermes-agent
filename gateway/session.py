...
class SessionStore:
    ...
    def get_or_create_session(
        ...
        # Seed new DM thread sessions with context.
        #
        # Two scenarios:
        # 1. Brand-new thread (first message): seed from the parent DM session
        #    so the user's original question + bot reply carry over.
        # 2. Auto-reset thread: seed from the thread's own expired transcript
        #    so the conversation context is preserved across resets.
        #
        # The old transcript data is still on disk (neither SQLite nor JSONL
        # files are deleted on reset), so we just load and copy it.
        if (
            source.chat_type == "dm"
            ...
            if was_auto_reset and db_end_session_id:
                # Scenario 2: re-seed from the thread's own expired transcript
                try:
                    old_history = self.load_transcript(db_end_session_id)
                    if old_history:
                        self.rewrite_transcript(entry.session_id, old_history)
                        logger.info(
                            "[Session] Re-seeded DM thread session %s with %d messages "
                            "from expired session %s",
                            entry.session_id, len(old_history), db_end_session_id,
                        )
                except Exception as e:
                    logger.warning("[Session] Failed to re-seed thread from expired session: %s", e)
            else:
                # Scenario 1: seed from parent DM session
